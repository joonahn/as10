...
fun yyAction28 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (Tokens.INT(make_int_args(yypos, yytext)))
      end
fun yyAction29 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (Tokens.ID(make_id_args(yypos, yytext)))
      end
fun yyAction30 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (ErrorMsg.error(make_pos(yypos, yytext), "ID syntax error"); continue())
      end
fun yyAction31 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (Tokens.PROJ(make_proj_args(yypos, yytext)))
      end
...
fun yyAction33 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error(make_pos(yypos, yytext), "proj number error"); continue())
      end
fun yyAction34 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN COMMENT;inc_comment(); continue()))
fun yyAction35 (strm, lastMatch : yymatch) = (yystrm := strm;
      (inc_comment(); continue()))
fun yyAction36 (strm, lastMatch : yymatch) = (yystrm := strm;
      (if dec_comment()=0 then YYBEGIN INITIAL else (); continue()))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction29(strm, yyNO_MATCH)
                      else yyQ26(strm', yyMATCH(strm, yyAction29, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction29(strm, yyNO_MATCH)
                  else yyQ26(strm', yyMATCH(strm, yyAction29, yyNO_MATCH))
            else if inp = #"`"
       ...
       of NONE => yyAction30(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp <= #"@"
                  then yyAction30(strm, yyNO_MATCH)
                  else yyQ61(strm', yyMATCH(strm, yyAction30, yyNO_MATCH))
            else if inp = #"a"
...
fun yyQ5 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction38(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ6(strm', yyMATCH(strm, yyAction38, yyNO_MATCH))
...
fun yyQ4 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction38(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ7(strm', yyMATCH(strm, yyAction38, yyNO_MATCH))
              else yyAction38(strm, yyNO_MATCH)
      (* end case *))
